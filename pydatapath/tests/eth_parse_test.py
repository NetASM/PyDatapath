...
def main():
    ...
    # where offset and length tell the location and size of the field in the raw packet. Second, we
    # apply the control policy which in this case is simple pass through.
    ...
    # Policy transformation to map on the hardware
    policy_transformed = (extract(eth_dstmac=(0, 0, 48),
                                  eth_srcmac=(0, 48, 48),
                                  eth_type=(0, 96, 16),
                                  payload=(0, 112, EOP)) >> \4
                          ((match(eth_srcmac=0x000000000001) >> modify(outport=2)) +
